...
def write_emodb_csv(emotions=["neutral", "calm", "happy", "sad", "angry", "fear", "disgust", "ps" , "boredom"], train_name="train_emo.csv",
    ...
    params:
        ...
        verbose (int/bool): verbosity level, 0 for silence, 1 for info, default is 1
    """
    target = {"path": [], "emotion": []}
    categories = {
        "W": "angry",
...
def write_tess_ravdess_csv(emotions=["angry", "fear", "disgust", "sad", "neutral", "happy", "ps"], train_name="train_tess_ravdess.csv",
...
def write_custom_csv(emotions=['neutral', 'calm', 'happy', 'sad', 'angry', 'fear', 'disgust', 'ps' , 'boredom'], train_name="train_custom.csv", test_name="test_custom.csv", train_size=0.55,
                    verbose=1):
    """
    ...
    for category in emotions:
        ...
        categories2 = {
            "1" : "neutral", "2" : "calm", "3" : "happy", "4" : "sad", "5" : "angry", "6" : "fear", "7" : "disgust", "8" : "ps"
        }
        ...
        total_files = glob.glob(f"data/custom/*.wav")
        ...
        for file in total_files :
            if "_" in os.path.basename(file) :
                ...
                try :
                    emotion = categories2[os.path.basename(file)[7]]
                except KeyError:
                    continue
                target['emotion'].append(emotion)
                target['path'].append(file) 
        ...
        if verbose:
            print("[Custom Dataset] Total files to write:", len(target['path']))             

        if verbose and total_files :
            print(f"[Custom Dataset] There are {len(total_files)} training audio files for category:{category}")
    
    n_samples = len(target['path'])
    test_size = int((1-train_size) * n_samples)
    train_size = int(train_size * n_samples)
    if verbose:
        print("[Custom Dataset] Training samples:", train_size)
        print("[Custom Dataset] Testing samples:", test_size)   
    X_train = target['path'][:train_size]
    X_test = target['path'][train_size:]
    y_train = target['emotion'][:train_size]
    y_test = target['emotion'][train_size:]
    ...
    """
    df1 = pd.read_csv('train_custom.csv')
    df1.drop_duplicates(subset="path")
    df1.to_csv('train_custom.csv')
    """

    pd.DataFrame({"path": X_test, "emotion": y_test}).to_csv(test_name)

    """
    df2 = pd.read_csv('test_custom.csv')
    df2.drop_duplicates(subset="path")
    df2.to_csv('test_custom.csv')
    """
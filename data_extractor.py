...
class AudioExtractor:
    ...
    def __init__(self, audio_config=None, verbose=1, features_folder_name="features", classification=True,
                    emotions=['neutral', 'calm', 'happy', 'sad', 'angry', 'fear', 'disgust', 'ps' , 'boredom'], balance=True):
        """
        Params:
    ...
    def load_metadata_from_desc_file(self, desc_files, partition):
        ...
        if not self.classification:
            ...
            elif len(self.emotions) == 9:
                self.categories = {'angry': 1, 'fear': 2, 'disgust': 3, 'sad': 4, 'boredom': 5, 'neutral': 6, 'calm': 7, 'happy': 8, 'ps': 9}
            else:
                raise TypeError("Regression is only for either ['sad', 'neutral', 'happy'] or ['neutral', 'calm', 'happy', 'sad', 'angry', 'fear', 'disgust', 'ps' , 'boredom']")
...
def load_data(train_desc_files, test_desc_files, audio_config=None, classification=True, shuffle=True,
                balance=True, emotions=['neutral', 'calm', 'happy', 'sad', 'angry', 'fear', 'disgust', 'ps' , 'boredom']):
